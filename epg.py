...
def fetch_programmes():
    ...
    for line in lines:
        ...
        if match:
            ...
            if len(line.replace(possible_time, "").strip()) < 12:   # πιο αυστηρός έλεγχος για γραμμές ώρας
...
def build_xml(programmes, today_date, tomorrow_date):
        ...
        for i, (time_str, title) in enumerate(programmes):
            try:
                ...
                if i < len(programmes) - 1:
                    ...
                    # Overnight correction (23:30 → 01:00)
                    if nh < h or (nh == h and nm < m):
                        stop_dt += timedelta(days=1)
                else:
                    stop_dt = start_dt + timedelta(hours=1)

                # Αν η έναρξη είναι νωρίς το πρωί (00:00-05:59) → επόμενη μέρα
                if h < 6:
                    start_dt += timedelta(days=1)
                    stop_dt += timedelta(days=1)

                # Διόρθωση λάθους 18:00-18:00 ή πολύ μικρής διάρκειας
                if prev_stop and (start_dt - prev_stop).total_seconds() < 60:
                    start_dt = prev_stop   # συνέχεια από προηγούμενο stop
    ...
    # Προσθήκη Πέμπτης + Παρασκευής (πλήρες)
    ...
    print(f"   → Πέμπτη: {today_date.strftime('%d/%m/%Y')}")
    print(f"   → Παρασκευή: {tomorrow_date.strftime('%d/%m/%Y')}")

def main():
    now = datetime.now()
    # Σήμερα = Πέμπτη, Αύριο = Παρασκευή
